package [(${model.packageName})].[# th:if="${model.moduleName}"][(${model.moduleName})].[/].controller;
...
public class [(${model.className})]Controller {
    ...
    public R<PageInfo<[(${model.className})]>> list([(${model.className})]Query query) {
        return R.success([(${model.className})]Service.search(query));
    }
    ...
    public R<Boolean> create(@Validated(Create.class) @RequestBody [(${model.className})]EditForm form, @ApiIgnore OperateInfo operateInfo) {
        return R.success([(${model.className})]Service.save(TransformUtils.transform(form, [(${model.className})].class)));
    }

    @ApiOperation("根据id更新[(${model.comment})]")
    @PutMapping("{id}")
    public R<Boolean> update(@PathVariable Long id, @Validated(Update.class) @RequestBody [(${model.className})]EditForm form) {
        form.setId(id);
        return R.success([(${model.className})]Service.updateByIdWithUser(TransformUtils.transform(form, [(${model.className})].class)));
    }

    @ApiOperation("[(${model.comment})]删除，根据ID集合(支持批量删除，如 /menu/1,2,3 逗号隔开 删除id为1和2和3的),需要注意的是不同游览器对url长度限制不一样，建议限制顶多删除一页的数据")
    @DeleteMapping("{ids}")
    public R<Boolean> delete(@PathVariable List<Long> ids, @ApiIgnore OperateInfo operateInfo) {
        return R.success([(${model.className})]Service.removeByIds(ids));
    }

    @ApiOperation("根据ID获取[(${model.comment})]记录")
    @GetMapping("{id}")
    public R<[(${model.className})]> searchById(@PathVariable Long id) {
        return R.success([(${model.className})]Service.getById(id));
    }
}
